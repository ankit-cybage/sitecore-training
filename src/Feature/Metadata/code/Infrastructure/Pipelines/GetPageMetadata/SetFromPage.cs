...
namespace SC91Training.Feature.Metadata.Infrastructure.Pipelines.GetPageMetadata
{
    public class SetFromPage
    {
        ...
        {
            var keywordsField = item.FieldHasValue(Templates.PageMetadata.Fields.Keywords) ? item[Templates.PageMetadata.Fields.Keywords] : String.Empty;
            foreach(var keyword in keywordsField.Split(','))
            {
